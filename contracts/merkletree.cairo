...
namespace MerkleTree {
    ...
    // ########################################################################################
    // The function calculates the root of the Merkle tree using the calculate_root function
    // and compares it to the provided merkle_root value.
    ...
    // >> Otherwise, the function calculates the next node in the Merkle tree
    // by concatenating the curr and proof values and hashing the result using the hash2 function,
    // which is imported from the starkware.cairo.common.hash module.
    // The function then recursively calls itself to calculate the root of the subtree rooted
    // at the current node, and returns the result.
    // ##
    // ref: https://github.com/ncitron/cairo-merkle-distributor/blob/master/contracts/merkle.cairo
    // @param curr
    // @param proof_len
